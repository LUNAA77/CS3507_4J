...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_text = article + highlights
        encodings = self.tokenizer(full_text, truncation=True, padding='max_length', max_length=512, return_tensors='pt')
        input_ids = encodings['input_ids'].squeeze()
        
        input_part = self.tokenizer(article, add_special_tokens=False)['input_ids']
        output_part = self.tokenizer(highlights, add_special_tokens=False)['input_ids']
                
        labels = torch.full_like(input_ids, -100)

        input_part_length = min(len(input_part), 512)
        output_part_length = min(len(output_part), 512 - input_part_length)

        labels[input_part_length:input_part_length + output_part_length] = torch.tensor(output_part[:output_part_length])

        if input_part_length + output_part_length > 512:
...
class SFTTrainer:
    ...
    def save_model(self, epoch):
        save_directory = os.path.join(self.save_path, f'sum_{epoch + 1}')
